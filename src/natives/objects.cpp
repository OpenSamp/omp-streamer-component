...
cell AMX_NATIVE_CALL Natives::SetDynamicObjectPos(AMX *amx, cell *params)
{
	...
	if (o != core->getData()->objects.end())
	{
		...
		for (std::unordered_map<int, Player>::iterator p = core->getData()->players.begin(); p != core->getData()->players.end(); ++p)
		{
			...
			{
				StreamerApi::SetPlayerObjectPos(p->first, i->second, o->second->position[0], o->second->position[1], o->second->position[2]);
			}
		}
...
cell AMX_NATIVE_CALL Natives::SetDynamicObjectRot(AMX *amx, cell *params)
{
	...
	if (o != core->getData()->objects.end())
	{
		...
		for (std::unordered_map<int, Player>::iterator p = core->getData()->players.begin(); p != core->getData()->players.end(); ++p)
		{
			...
			if (i != p->second.internalObjects.end())
			{
				StreamerApi::SetPlayerObjectRot(p->first, i->second, o->second->rotation[0], o->second->rotation[1], o->second->rotation[2]);
			}
		}
		if (o->second->move)
		{
...
cell AMX_NATIVE_CALL Natives::AttachCameraToDynamicObject(AMX *amx, cell *params)
{
	...
	if (p != core->getData()->players.end())
	{
		...
		{
			StreamerApi::AttachCameraToPlayerObject(p->first, internalId);
			return 1;
		}
	}
	return 0;
...
cell AMX_NATIVE_CALL Natives::EditDynamicObject(AMX *amx, cell *params)
{
	...
	if (p != core->getData()->players.end())
	{
		...
		if (internalId != INVALID_OBJECT_ID)
		{
			StreamerApi::EditPlayerObject(p->first, internalId);
			return 1;
		}
	}
	return 0;
...
cell AMX_NATIVE_CALL Natives::SetDynamicObjectMaterial(AMX *amx, cell *params)
{
	...
	if (o != core->getData()->objects.end())
	{
		...
		for (std::unordered_map<int, Player>::iterator p = core->getData()->players.begin(); p != core->getData()->players.end(); ++p)
		{
			...
			if (i != p->second.internalObjects.end())
			{
				StreamerApi::SetPlayerObjectMaterial(p->first, i->second, index, o->second->materials[index].main->modelId, o->second->materials[index].main->txdFileName.c_str(), o->second->materials[index].main->textureName.c_str(), o->second->materials[index].main->materialColor);
			}
		}
...
cell AMX_NATIVE_CALL Natives::GetPlayerCameraTargetDynObject(AMX *amx, cell *params)
{
	...
	if (p != core->getData()->players.end())
	{
		int objectid = StreamerApi::GetPlayerCameraTargetObject(p->second.playerId);
